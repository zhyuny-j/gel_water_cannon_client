...
void OnLogoutButtonClick(HWND hWnd);
void CheckNetworkStatus(HWND hWnd);
...
static BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	// Set the timer to check network status
	SetTimer(hWnd, NETWORK_CHECK_TIMER, NETWORK_CHECK_INTERVAL, NULL);
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		if (!IsClientConnected()) {
			DisplayMessageOkBox("No connection to server. Connect to the server first.");
			return 0;
		}
	...
	case WM_TIMER:
		if (wParam == NETWORK_CHECK_TIMER) {
			CheckNetworkStatus(hWnd);
		}
		break;
	...
	case WM_LOGIN_STATE:
	{
		...
		unsigned int privilege = static_cast<unsigned int>(wParam);
		if (loginState == SUCCESS) {
			...
			DisplayMessageOkBox("Your account cannot be found.");
		}
	...
	case WM_SYSTEM_STATE:
	{
		...
		if (SystemState & CAMERA_ON)
			CheckDlgButton(hWnd, IDC_CHECKBOX_CAMERA, BST_CHECKED);
		else
			CheckDlgButton(hWnd, IDC_CHECKBOX_CAMERA, BST_UNCHECKED);
...
void OnLoginButtonClick(HWND hWnd) {
	...
	if (strlen(userId) == 0 || strlen(userPw) == 0) {
		DisplayMessageOkBox("Input ID or Password");
		return;
	}
...
void OnLogoutButtonClick(HWND hWnd) {
	...
	SetWindowTextA(GetDlgItem(hWnd, IDC_STATIC_LOGGEDIN_ID), "");

	// 로그인 창 필드 초기화
	SetWindowTextA(GetDlgItem(hWnd, IDC_EDIT_ID), "");
	SetWindowTextA(GetDlgItem(hWnd, IDC_EDIT_PASSWORD), "");
